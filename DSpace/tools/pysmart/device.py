...
class Device(object):
    ...
    """
    def __init__(self, name, interface=None, ssh=None):
        ...
        **(bool):** True if the device supports SMART (or SCSI equivalent) and
        ...
        """**(str):** SMART health self-assessment as reported by
        the device."""
        self.messages = []
        """
    ...
    def get_selftest_result(self, output=None):
        ...
        * **(str):** Estimated completion time of a test in-progess, if known.
        ...
        # appearing
        # identical. Comparing the length of the log adds some confidence
        # until it maxes, as above. Comparing the least-recent test result
        # greatly diminishes the chances that two sets of two tests each were
        # run within an hour of themselves, but with 16-17 other tests run in
        # between them.
        if self.tests is not None:
